				...
				for (Field f : fs) {
					...
					if (!modifier.startsWith("private") || modifier.split(" ").length != 3) {
						...
						try {
							...
							if (fieldValue == null && g != null && StringUtils.isNotBlank(g.generator())) {
								Map<String, Object> r = (Map<String, Object>) DatabaseConfig
										.query(g.generator().trim());
								if (r != null && !r.isEmpty()) {
									for (String key : r.keySet()) {
										fieldValue = r.get(key);
										break;
									}
								}
							}
