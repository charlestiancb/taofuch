...
public class WarMojo extends AbstractWarMojo {
	...
	/**
	 * Whether or not to fail the build if the <code>web.xml</code> file is
	 * missing. Set to <code>false</code> if you want you WAR built without a
	 * <code>web.xml</code> file. This may be useful if you are building an
	 * overlay that has no web.xml file.
	 * 
	...
	private void performPackaging(File warFile)	throws IOException,
		...
		if (!failOnMissingWebXml) {
			...
			// The flag is wrong in plexus-archiver so it will need to be fixed
			// at some point
